...
@Suite("Mac Remote Play pairing", .serialized)
@MainActor
struct MacRemotePlayPairingTests {
    ...
    func localNetworkPreflightRunsExactlyOnce() async {
        ...
        let model = MacPlayStationPairingModel(
    ...
    func failedPreflightStillAllowsPairing() async throws {
        ...
        let fixture = makeFixture(
            startups: [
                MacStartupSnapshot(consoles: []),
                MacStartupSnapshot(consoles: [summary(saved)]),
            ],
            effects: effects
        )
        await fixture.coordinator.prepare()
    ...
    func accountIdentityCapabilityAndForwarding() async throws {
        ...
        let unavailableModel = MacPlayStationPairingModel(
        ...
        let availableModel = MacPlayStationPairingModel(
    ...
    @Test("Account acquisition clears a manual draft and pairs with the acquired identity")
    func acquiredIdentityReplacesManualDraft() async throws {
        let identity = try testIdentity()
        let effects = PairingEffects(
            identityOutcome: .success(identity),
            pairOutcome: .success(testSavedConsole())
        )
        let fixture = makeFixture(effects: effects)
        await fixture.coordinator.prepare()
        let model = makeReadyModel(coordinator: fixture.coordinator)
        await model.prepareLocalNetwork()
        model.manualAccountID = "123456789"
        model.manualAccountDisplayName = "Draft"

        await model.acquireAccountIdentity()
        #expect(model.manualAccountID.isEmpty)
        #expect(model.manualAccountDisplayName.isEmpty)
        #expect(model.hasAccountIdentity)
        #expect(model.canPair)
        await model.pair()
        #expect(await effects.pairRequests.first?.accountID == identity.accountID)
    }

    @Test("Editing a prepared Account ID disables pairing until the new ID is applied")
    func editingManualAccountIDInvalidatesPreviousIdentity() async {
        let effects = PairingEffects(identityCapability: .unavailable)
        let fixture = makeFixture(effects: effects)
        await fixture.coordinator.prepare()
        let model = makeReadyModel(coordinator: fixture.coordinator)
        ...
        model.manualAccountID = "987654321"
        #expect(model.hasAccountIdentity == false)
        ...
        #expect(model.hasAccountIdentity)
        #expect(model.canPair)
        model.manualAccountID = ""
        #expect(model.canPair == false)
    }

    @Test("Closing manual entry clears the Account ID and prepared identity")
    func manualAccountEntryIsTransient() async {
        let fixture = makeFixture(effects: PairingEffects(identityCapability: .unavailable))
        await fixture.coordinator.prepare()
        let model = makeReadyModel(coordinator: fixture.coordinator)
        model.manualAccountID = "123456789"
        model.applyManualAccountID()
    ...
    func hostAndPINValidation() async throws {
        ...
        let fixture = makeFixture(
            startups: [
                MacStartupSnapshot(consoles: []),
                MacStartupSnapshot(consoles: [pairedSummary]),
            ],
            effects: effects
        )
        await fixture.coordinator.prepare()
    ...
    func ordinaryFailureClearsOnlyPIN() async throws {
        ...
        #expect(model.successConfirmation == nil)
        #expect(fixture.coordinator.pairingSecureSaveIsPending == false)
    }

    @Test("Success confirmation appears only after the secure save is verified")
    func successConfirmationWaitsForVerifiedSecureSave() async throws {
        let saved = testSavedConsole(name: "Ready PS5")
        let pairGate = PairingTestGate()
        ...
        #expect(model.status == .saving)
        #expect(model.successConfirmation == nil)
        await retryGate.open()
        await retry.value

        let confirmation = try #require(model.successConfirmation)
        #expect(confirmation.consoleName == saved.displayName)
        #expect(confirmation.consoleID == saved.id)
        #expect(model.hasAccountIdentity == false)
        #expect(model.accountDisplayName == nil)
        #expect(model.manualAccountID.isEmpty)
        #expect(model.manualAccountDisplayName.isEmpty)
        #expect(model.linkDevicePIN.isEmpty)
        #expect(model.inputsAreEditable == false)
        #expect(await effects.pairCount == 1)
    ...
    func secureSaveRetryDoesNotPairAgain() async throws {
        ...
        let fixture = makeFixture(
            startups: [
                MacStartupSnapshot(consoles: []),
                MacStartupSnapshot(consoles: [summary(saved), summary(alpha)]),
            ],
            effects: effects
        )
        await fixture.coordinator.prepare()
    ...
    func successfulPairClearsTransientsAndReloadsConsoles() async throws {
        ...
        #expect(
            model.successConfirmation == MacPlayStationPairingSuccess(
                consoleName: "Zulu",
                consoleID: saved.id
            )
        )
    ...
    func recoveryTargetRequiresCanonicalConsoleID() {
        ...
        #expect(
            MacPairingTarget.canonicalRecoveryConsoleID(
    ...
    func connectWaitsForPairingSheetDismissal() {
        let consoleID = UUID()
        var handoff = MacPairingConnectHandoff()
    ...
    func activeSessionGuardsRegistrationMutations() async throws {
        ...
        let effects = PairingEffects()
        let fixture = makeFixture(
            startups: [MacStartupSnapshot(consoles: [console])],
            effects: effects,
            session: session
        )
        await fixture.coordinator.prepare()
    ...
    func removeReloadsConsoles() async throws {
        ...
        let fixture = makeFixture(
            startups: [
                MacStartupSnapshot(consoles: [removed]),
...
private extension MacRemotePlayPairingTests {
    struct Fixture {
    ...
    func makeFixture(
        startups: [MacStartupSnapshot] = [MacStartupSnapshot(consoles: [])],
        ...
        let suiteName = "RemotePlayMacPairingTests.\(UUID().uuidString)"
    ...
    func makeReadyModel(
        coordinator: MacRemotePlayCoordinator,
        target: MacPairingTarget = MacPairingTarget()
    ) -> MacPlayStationPairingModel {
        let model = MacPlayStationPairingModel(coordinator: coordinator, target: target)
        model.hostAddress = "192.0.2.50"
        model.linkDevicePIN = "01234567"
        return model
    }

    func prepareForPairing(_ model: MacPlayStationPairingModel) async {
        await model.prepareLocalNetwork()
        await model.acquireAccountIdentity()
        #expect(model.canPair)
    }
    ...
    func summary(_ console: SavedPlayStationConsole) -> MacConsoleSummary {
        MacConsoleSummary(
            id: console.id,
    ...
    func catchesSessionActive(
        ...
        do {
            try await operation()
            return false
        } catch let error as MacRemotePlayCoordinatorError {
...
private final class PairingTestControllerSource:
    MacRemotePlayControllerSource, @unchecked Sendable {
    func snapshot() -> ControllerSnapshot { .neutral }
    func connectionSnapshot() -> MacControllerConnection { .disconnected }
}

private final class PairingTestSession: MacRemotePlaySession, @unchecked Sendable {
    let id = UUID()
    let videoSurface: MacRemotePlayVideoSurface? = .testHarness
    ...
    func snapshot() async -> MacRemoteSessionSnapshot {
        MacRemoteSessionSnapshot(state: .disconnected, displayIsBlocked: false)
    }
